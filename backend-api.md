...
| GET | `/api/devices/{device_id}` | Device details with points (incl. `protocol`, KNX `group_address`/`dpt`) |
...
| GET | `/api/protocols` | Protocol cards: id, label, live device count |
| GET | `/api/{protocol}/config/schema` | Per-protocol scan-configuration field spec |
| POST | `/api/{protocol}/discover` | Start a live discovery scan → `scan_id` |
| GET | `/api/{protocol}/discover/{scan_id}` | Poll a scan: status + discovered devices |
| GET | `/api/{protocol}/discover/{scan_id}/devices/{ref}/objects` | Per-device object/point detail |
| POST | `/api/assets/{asset_id}/devices` | Add selected discovered devices, attach to asset |
...
## Device Discovery (per-protocol)

Live discovery + onboarding wizard. Each protocol is reachable under its own
namespace — `/api/<protocol>/...` — sharing one contract (verb, request/response
shape, auth, error format); only the path segment and the scan-config fields
differ. Protocols: `bacnet`, `modbus`, `mqtt`, `knx`.

Flow: **select protocol → fetch its config schema → start scan → poll for
devices → add selected devices to an asset.** Scans run in the background
because network probes are slow.

### GET /api/protocols

Protocol cards for the wizard (id, label, current device count).

```bash
curl -u admin:admin123 https://bacnet.tools.thefusionapps.com/api/protocols
```

```json
{"protocols": [
  {"protocol": "bacnet", "label": "BACnet/IP", "device_count": 8, "available": true},
  {"protocol": "modbus", "label": "Modbus TCP", "device_count": 0, "available": true},
  {"protocol": "mqtt",   "label": "MQTT",       "device_count": 0, "available": true},
  {"protocol": "knx",    "label": "KNX",        "device_count": 3, "available": true}
]}
```

### GET /api/{protocol}/config/schema

The scan-configuration fields for one protocol — render the form from this.

```bash
curl -u admin:admin123 https://bacnet.tools.thefusionapps.com/api/bacnet/config/schema
```

```json
{"protocol": "bacnet", "schema": {"fields": [
  {"name": "scan_mode", "label": "Scan Mode", "type": "select", "options": ["broadcast","ip_range"], "default": "broadcast", "required": true},
...
], "notes": "BACnet uses Who-Is broadcast discovery; IP range narrows the subnet."}}
```

Per-protocol config fields:

| Protocol | Fields |
|----------|--------|
| `bacnet` | `scan_mode` (broadcast/ip_range), `ip_range_from`, `ip_range_to`, `port` (47808), `timeout_s`, `retries`, `deep_scan` |
| `modbus` | `host`, `port` (502), `unit_start`, `unit_end`, `timeout_s`, `deep_scan` |
| `mqtt`   | `host`, `port` (1883), `prefix`, `username`, `password`, `window_s` |
| `knx`    | `mode` (ets_file/gateway), `ets_file_b64`, `ets_filename`, `password`, `gateway_ip`, `gateway_port` |

### POST /api/{protocol}/discover

Start a scan with the form values as the JSON body. Returns a `scan_id` to poll.

```bash
curl -u admin:admin123 -X POST \
  https://bacnet.tools.thefusionapps.com/api/bacnet/discover \
  -H "Content-Type: application/json" \
  -d '{"scan_mode":"ip_range","ip_range_from":"192.168.10.1","ip_range_to":"192.168.10.255","port":47808,"timeout_s":5,"retries":2,"deep_scan":true}'
```

Response `200`: `{"scan_id": "a041f7272e25", "protocol": "bacnet", "status": "running"}`

KNX from an ETS file (base64-encode the `.knxproj`/`.xml`/`.csv`):

```bash
curl -u admin:admin123 -X POST \
  https://bacnet.tools.thefusionapps.com/api/knx/discover \
  -H "Content-Type: application/json" \
  -d '{"mode":"ets_file","ets_filename":"project.knxproj","ets_file_b64":"<BASE64>","password":""}'
```

`400` with `{"detail": "..."}` on bad config (e.g. unreachable broker, wrong ETS password).

### GET /api/{protocol}/discover/{scan_id}

Poll the scan: status (`running` | `done` | `error`) plus the discovered devices.

```bash
curl -u admin:admin123 \
  https://bacnet.tools.thefusionapps.com/api/bacnet/discover/a041f7272e25
```

```json
{
  "scan_id": "a041f7272e25",
  "protocol": "bacnet",
  "status": "done",
  "error": null,
  "count": 2,
  "devices": [
    {"ref": "192.168.10.9-300", "protocol": "bacnet", "name": "AHU Controller",
     "address": "192.168.10.9", "device_id": 300, "vendor": "Schneider Electric",
     "model": "", "object_count": 187}
  ]
}
```

On failure: `status: "error"`, `error: "Modbus discovery failed: Could not connect ..."`, `devices: []`.

### GET /api/{protocol}/discover/{scan_id}/devices/{ref}/objects

Per-device object/point detail (for the mapping step). KNX objects carry
`group_address` + `dpt`.

```bash
curl -u admin:admin123 \
  "https://bacnet.tools.thefusionapps.com/api/knx/discover/<scan_id>/devices/knx-ets/objects"
```

```json
{"ref": "knx-ets", "count": 13, "objects": [
  {"object_name": "CH-1 - Switching : OnOff", "object_type": "1",
   "group_address": "0/0/1", "dpt": "1.001", "present_value": 0, "units": ""}
]}
```

### POST /api/assets/{asset_id}/devices

Add the selected discovered devices: persists each as a `Device`, brings it
online on the network engine, and registers it with TimescaleDB. `refs` are the
`ref` values of the checked devices.

```bash
curl -u admin:admin123 -X POST \
  https://bacnet.tools.thefusionapps.com/api/assets/AHU-01/devices \
  -H "Content-Type: application/json" \
  -d '{"scan_id":"a041f7272e25","refs":["192.168.10.9-300","knx-ets"]}'
```

```json
{"asset_id": "AHU-01", "added": 1, "results": [
  {"ref": "knx-ets", "added": true, "device_id": 7004, "name": "KNX ETS Import", "points": 13}
]}
```

`400` if `scan_id`/`refs` missing or the scan is unknown.
